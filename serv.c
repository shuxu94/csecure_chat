...
struct clientdata {
	...
	char pubkey[MESSSIZE]; //for client public key
};
...
int main(int argc, char **argv)
{
	...
	while(1)
	{
		...
		if(newclient == 1) //if new client, add to list of clients
		{
			...
			rsa_pkey_file = fopen(argv[1], "rb"); //get public key for server
			...
			char packet3[MESSSIZE]; //receive client's public key
			bzero(packet3, sizeof(packet3));
			if(recvfrom(sockfd, packet3, sizeof(packet3), 0, (struct sockaddr *)&rmaddr, &addrlen) < 0)
			{ 
			...
			strncpy(clientlist[clientnum].pubkey, packet3, strlen(packet3)); //save client key
			...
			if(sendto(sockfd, pkey, strlen(pkey), 0, (struct sockaddr *) &rmaddr, addrlen) < 0) //send public key to client
			{ 
				perror("The sendto command failed."); 
				return 0; 
			}
		...
		memcpy(&newkey, buf, sizeof(newkey));

		FILE *secret_key_file;
		char plaintext[MESSSIZE];
			...
			if(buf[1] == 'u')
			{
				...
				else //otherwise, just change the client's username
				{
					/*if(!(strncmp(buf, clientlist[i].username, strlen(buf))))
					{
						...
						char* test4 = "Username already in use. Please disconnect.\n";
					...
					//NEED TO RUN LOOP OVER ALL CLIENTS TO CHECK USERNAMES					
